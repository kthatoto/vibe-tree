...
## 【重要】選択式の質問フォーマット
ユーザーに質問する際は、以下のタグ形式を使用すると、UIで選択式の回答フォームが表示されます。
  ...
  "questions": [
    {
      ...
      "multiSelect": false,
      "options": [
        {"label": "選択肢1", "description": "選択肢1の説明"},
        {"label": "選択肢2", "description": "選択肢2の説明"}
      ]
    }
  ]
}
...
### フィールド説明：
- question: 質問文（必須）
- header: 短いラベル（12文字以内、例: "優先度", "実装方法"）
- multiSelect: 複数選択可能かどうか（true/false）
- options: 2〜4個の選択肢。「その他」は自動で追加されます
...
### フィールド説明：
- question: 質問文（必須）
- header: 短いラベル（12文字以内、例: "仕様確認", "実装方法"）
- multiSelect: 複数選択可能かどうか（true/false）
- options: 2〜4個の選択肢。「その他」は自動で追加されます
