...
ユーザーに質問する際は、**必ず AskUserQuestion ツールを使用してください**。
自然言語で質問するのではなく、選択肢形式で質問することで、ユーザーが簡単に回答できます。
...
- 2〜4個の選択肢を用意する
- 各選択肢には簡潔な説明をつける
- 複数選択が必要な場合は multiSelect: true を使う
- 「その他」は自動で追加されるので、選択肢には含めない
...
- 具体的な改善案を提示する
- 良い点も指摘する
- 質問は AskUserQuestion ツールで1-2問に絞る
